...
exports.createActiveGame = (req, res) => {
    ...
    let { wp, bp, timer, startedTime } = req.body
    ...
    if (!timer || !startedTime) {
        return res.status(400).send({
            message: 'Need info about time format and started time'
        })
    } 
...
exports.updateActiveGame = (req, res) => {
    ...
    let { wp, bp, result: gameResult, move, time, i1, i2 } = req.body
    ...
    .then(() => {
        ...
        handleGameFinished({ gameID, gameResult, record: update.record, timer: update.timer })
...
handleGameFinished = ({ gameID, gameResult, record, timer }) => {
    return queryExec('update active_game set result=?', [gameResult])
    .then(() => { 
